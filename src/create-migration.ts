...
async function createMigrationFile(dbName: DbName, name: string) {
  const filepath = path.join(getMigrationFolder(dbName), `${createdAt()}-${name}.ts`);
...
const dbName = process.argv
  .find(arg => arg.startsWith("--dbname"))?.split("=")[1] as DbName;
const name = process.argv
  .find(arg => arg.startsWith("--name"))?.split("=")[1] as string;
